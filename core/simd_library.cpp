...
namespace simd
{
  ...
  {
    if (buf_x_ != nullptr)
      custom_aligned_free((void *)buf_x_);
    if (buf_y_ != nullptr)
      custom_aligned_free((void *)buf_y_);
    if (buf_z_ != nullptr)
      custom_aligned_free((void *)buf_z_);

    if (buf_x_warped_ != nullptr)
      custom_aligned_free((void *)buf_x_warped_);
    if (buf_y_warped_ != nullptr)
      custom_aligned_free((void *)buf_y_warped_);
    if (buf_z_warped_ != nullptr)
      custom_aligned_free((void *)buf_z_warped_);
  }

  void PointWarper::Warp(
      const std::vector<Eigen::Vector3f> &points, const Eigen::Isometry3f &pose,
      std::vector<Eigen::Vector3f> &warped_points)
  {
    ...
    for (; idx < num_points; idx += _SIMD_BYTE_STEP)
    {
      ...
      _SIMD_TYPE _x_warp = _SIMD_ADD_PS(_SIMD_ADD_PS(_SIMD_ADD_PS(_SIMD_MUL_PS(_r11, _x), _SIMD_MUL_PS(_r12, _y)), _SIMD_MUL_PS(_r13, _z)), _t1);
      _SIMD_TYPE _y_warp = _SIMD_ADD_PS(_SIMD_ADD_PS(_SIMD_ADD_PS(_SIMD_MUL_PS(_r21, _x), _SIMD_MUL_PS(_r22, _y)), _SIMD_MUL_PS(_r23, _z)), _t2);
      _SIMD_TYPE _z_warp = _SIMD_ADD_PS(_SIMD_ADD_PS(_SIMD_ADD_PS(_SIMD_MUL_PS(_r31, _x), _SIMD_MUL_PS(_r32, _y)), _SIMD_MUL_PS(_r33, _z)), _t3);

      _x_warp = _SIMD_ADD_PS(_SIMD_ADD_PS(_SIMD_ADD_PS(_SIMD_MUL_PS(_r11, _x_warp), _SIMD_MUL_PS(_r12, _y_warp)), _SIMD_MUL_PS(_r13, _z_warp)), _t1);
      _y_warp = _SIMD_ADD_PS(_SIMD_ADD_PS(_SIMD_ADD_PS(_SIMD_MUL_PS(_r21, _x_warp), _SIMD_MUL_PS(_r22, _y_warp)), _SIMD_MUL_PS(_r23, _z_warp)), _t2);
      _z_warp = _SIMD_ADD_PS(_SIMD_ADD_PS(_SIMD_ADD_PS(_SIMD_MUL_PS(_r31, _x_warp), _SIMD_MUL_PS(_r32, _y_warp)), _SIMD_MUL_PS(_r33, _z_warp)), _t3);
  ...
  //   // __m128 rrrr = _mm_SIMD_ADD_PS(_mm_SIMD_MUL_PS(gx4, _mm_sub_ps(u_warp4, u_match4)), _mm_SIMD_MUL_PS(gy4, _mm_sub_ps(v_warp4, v_match4)));
