...
void so3Exp(const double w1, const double w2, const double w3, Eigen::Matrix<double, 3, 3> &R)
{
    // initialize variables
    double theta = 0.0;
    Eigen::Matrix3d wx;

    theta = std::sqrt(w1 * w1 + w2 * w2 + w3 * w3);
    wx << 0, -w3, w2,
        w3, 0, -w1,
        -w2, w1, 0;
...
void so3Exp(const Eigen::Matrix<double, 3, 1> &w, Eigen::Matrix<double, 3, 3> &R)
{
    // initialize variables
    double theta = 0.0;
    Eigen::Matrix3d wx;
...
void so3Exp_f(const Eigen::Matrix<float, 3, 1> &w, Eigen::Matrix3f &R)
{
    // initialize variables
    double theta = 0.0;
    Eigen::Matrix3f wx;
    ...
    Eigen::Matrix3f wxwx = wx * wx;
    if (theta < 1e-9)
    {
        R = Eigen::Matrix3f::Identity() + wx + 0.5 * wxwx;
    }
    else
    {
        double invtheta2 = 1.0 / (theta * theta);
        R = Eigen::Matrix3f::Identity() + (sin(theta) / theta) * wx + ((1 - cos(theta)) * invtheta2) * wxwx;
    }
}

void SO3Log(const Eigen::Matrix<double, 3, 3> &R, Eigen::Matrix<double, 3, 1> &w)
{
    Eigen::Matrix<double, 3, 3> wx;
    Eigen::Matrix<double, 3, 3> lnR;
...
void SO3Log_f(const Eigen::Matrix<float, 3, 3> &R, Eigen::Matrix<float, 3, 1> &w)
{
    Eigen::Matrix<double, 3, 3> wx;
    Eigen::Matrix<double, 3, 3> lnR;
    ...
    else
    {
        ...
        lnR = (theta / (2.0 * sin(theta))) * (R.cast<double>() - R.cast<double>().transpose());
        w << -lnR(1, 2), lnR(0, 2), -lnR(0, 1);
    }
}
