...
bool PoseOnlyBundleAdjustmentSolver::Solve_Monocular_6Dof(const std::vector<Eigen::Vector3f> &reference_position_list,
                                                          const std::vector<Eigen::Vector2f> &matched_pixel_list,
                                                          const float fx, const float fy, const float cx,
                                                          const float cy, Eigen::Isometry3f &reference_to_current_pose,
                                                          std::vector<bool> &mask_inlier, Options options,
                                                          Summary *summary) {
  ...
  if (reference_position_list.size() != matched_pixel_list.size())
    throw std::runtime_error(
        "In PoseOnlyBundleAdjustmentSolver::SolveMonocularPoseOnlyBundleAdjustment6Dof(), world_position_list.size() "
        "!= current_pixel_list.size()");
  ...
  for (int iteration = 0; iteration < MAX_ITERATION; ++iteration) {
    ...
    WarpPositionList(pose_camera_to_world_optimized, reference_position_list, warped_local_position_list);
    float err_curr = 0.0f;
    ...
    for (size_t index = 0; index < n_pts; ++index) {
      ...
      ComputeJacobianResidual_ReprojectionError_6Dof(local_position, matched_pixel, fx, fy, cx, cy,
      ...
      ComputeGradientHessian_ReprojectionError_6Dof(jacobian_transpose_u, jacobian_transpose_v, residual_vector,
                                                    THRES_HUBER, gradient_i, hessian_i, error_i, error_nonweighted_i);
    ...
    for (size_t i = 0; i < 6; ++i) JtWJ(i, i) *= (1.0f + lambda);  // lambda
    ...
    if (iteration == MAX_ITERATION - 1) is_converged = false;
...
bool PoseOnlyBundleAdjustmentSolver::Solve_Stereo_6Dof(
    ...
    const std::vector<Eigen::Vector2f> &matched_right_pixel_list, const float fx_left, const float fy_left,
    const float cx_left, const float cy_left, const float fx_right, const float fy_right, const float cx_right,
    const float cy_right, const Eigen::Isometry3f &left_to_right_pose,
    Eigen::Isometry3f &reference_to_current_left_pose, std::vector<bool> &mask_inlier_left,
    std::vector<bool> &mask_inlier_right, Options options, Summary *summary) {
  ...
  if (reference_position_list.size() != matched_left_pixel_list.size())
    throw std::runtime_error(
        "In PoseOnlyBundleAdjustmentSolver::SolveStereoPoseOnlyBundleAdjustment6Dof(), world_position_list.size() != "
        "left_current_pixel_list.size()");
  if (reference_position_list.size() != matched_right_pixel_list.size())
    throw std::runtime_error(
        "In PoseOnlyBundleAdjustmentSolver::SolveStereoPoseOnlyBundleAdjustment6Dof(), world_position_list.size() != "
        "right_current_pixel_list.size()");
  ...
  for (int iter = 0; iter < MAX_ITERATION; ++iter) {
    ...
    // Warp and project 3d point & calculate error
    WarpPositionList(pose_camera_to_world_optimized, reference_position_list, warped_left_position_list);
    WarpPositionList(pose_right_to_left, warped_left_position_list, warped_right_position_list);
    ...
    for (size_t index = 0; index < n_pts; ++index) {
      ...
      ComputeJacobianResidual_ReprojectionError_6Dof(left_local_position, matched_left_pixel, fx_left, fy_left, cx_left,
                                                     cy_left, jacobian_transpose_u_left, jacobian_transpose_v_left,
                                                     residual_vector_left);
      ...
      ComputeGradientHessian_ReprojectionError_6Dof(jacobian_transpose_u_left, jacobian_transpose_v_left,
                                                    residual_vector_left, THRES_HUBER, gradient_i_left, hessian_i_left,
                                                    error_i_left, error_nonweighted_i_left);
      ...
      // Check right
      if (matched_right_pixel.x() < 0 || matched_right_pixel.y() < 0) continue;
      ...
      ComputeJacobianResidual_ReprojectionError_6Dof(right_local_position, matched_right_pixel, fx_right, fy_right,
                                                     cx_right, cy_right, jacobian_transpose_u_right,
                                                     jacobian_transpose_v_right, residual_vector_right);
      ...
      ComputeGradientHessian_ReprojectionError_6Dof(jacobian_transpose_u_right, jacobian_transpose_v_right,
                                                    residual_vector_right, THRES_HUBER, gradient_i_right,
                                                    hessian_i_right, error_i_right, error_nonweighted_i_right);
    ...
    if (iter == MAX_ITERATION - 1) is_converged = false;
...
bool PoseOnlyBundleAdjustmentSolver::Solve_Monocular_Planar3Dof(
    const std::vector<Eigen::Vector3f> &world_position_list, const std::vector<Eigen::Vector2f> &matched_pixel_list,
    const float fx, const float fy, const float cx, const float cy, const Eigen::Isometry3f &pose_base_to_camera,
    const Eigen::Isometry3f &pose_world_to_last, Eigen::Isometry3f &pose_world_to_current,
    std::vector<bool> &mask_inlier, Options options, Summary *summary) {
  ...
  if (world_position_list.size() != matched_pixel_list.size())
    throw std::runtime_error(
        "In PoseOnlyBundleAdjustmentSolver::SolveMonocularPoseOnlyBundleAdjustment3Dof(), world_position_list.size() "
        "!= current_pixel_list.size()");
  ...
  const Eigen::Isometry3f pose_c2c1_prior = pose_world_to_current.inverse() * pose_world_to_last;
  ...
  for (int iteration = 0; iteration < MAX_ITERATION; ++iteration) {
    ...
    pose_b2b1_optimized.linear() << cos_psi, -sin_psi, 0, sin_psi, cos_psi, 0, 0, 0, 1;
    ...
    // Warp and project 3d point & calculate error
    WarpPositionList(pose_c2b1_optimized, world_position_list, warped_local_position_list);
    float err_curr = 0.0f;
    size_t count_invalid = 0;
    for (size_t index = 0; index < n_pts; ++index) {
      const Eigen::Vector2f &matched_pixel = matched_pixel_list[index];
      ...
      ComputeJacobianResidual_ReprojectionError_Planar3Dof(local_position, world_position, matched_pixel, fx, fy, cx,
                                                           cy, R_cb, cos_psi, sin_psi, jacobian_transpose_u,
                                                           jacobian_transpose_v, residual_i);

      Eigen::Matrix<float, 3, 3> hessian_i;
      Eigen::Matrix<float, 3, 1> gradient_i;
      float error_i = 0;
      float error_nonweighted_i = 0;
      ComputeGradientHessian_ReprojectionError_Planar3Dof(jacobian_transpose_u, jacobian_transpose_v, residual_i,
                                                          THRES_HUBER, gradient_i, hessian_i, error_i,
                                                          error_nonweighted_i);
    ...
    for (size_t i = 0; i < 3; ++i) JtWJ(i, i) *= (1.0f + lambda);  // lambda
    ...
    if (delta_param.norm() < THRES_DELTA_XI || delta_error < THRES_DELTA_ERROR) {
      is_converged = true;
      // std::cout << "    Early convergence. stops at iteration " << iter << ", error: " << err_curr << ", delta_error:
      // " << delta_error << ", step_size: " << delta_xi.norm() << ", # invalid: " << count_invalid << "\n";
      break;
    }
...
bool PoseOnlyBundleAdjustmentSolver::Solve_Stereo_Planar3Dof(
    ...
    const std::vector<Eigen::Vector2f> &matched_right_pixel_list, const float fx_left, const float fy_left,
    const float cx_left, const float cy_left, const float fx_right, const float fy_right, const float cx_right,
    const float cy_right, const Eigen::Isometry3f &base_to_camera_pose, const Eigen::Isometry3f &left_to_right_pose,
    const Eigen::Isometry3f &world_to_last_pose, Eigen::Isometry3f &world_to_current_pose,
    std::vector<bool> &mask_inlier_left, std::vector<bool> &mask_inlier_right, Options options, Summary *summary) {
  timer::StopWatch stopwatch("SolveMonocularPoseOnlyBundleAdjustment3Dof");
  const auto &max_iteration = options.iteration_handle.max_num_iterations;
  const auto &threshold_convergence_delta_error = options.convergence_handle.threshold_cost_change;
  const auto &threshold_convergence_delta_pose = options.convergence_handle.threshold_step_size;
  ...
  // Warp the world position to the last frame
  const Eigen::Isometry3f pose_c2c1_prior = world_to_current_pose.inverse() * world_to_last_pose;
  ...
  for (int iteration = 0; iteration < MAX_ITERATION; ++iteration) {
    ...
    // Warp and project 3d point & calculate error
    WarpPositionList(pose_left_c2b1_optimized, world_position_list, warped_left_position_list);
    WarpPositionList(pose_right_c2b1_optimized, world_position_list, warped_right_position_list);
    ...
    for (size_t index = 0; index < n_pts; ++index) {
      ...
      ComputeJacobianResidual_ReprojectionError_Planar3Dof(
          ...
          rotation_left_camera_to_base, cos_psi, sin_psi, jacobian_transpose_u_left, jacobian_transpose_v_left,
          residual_vector_left);
      ...
      float error_i_left = 0;
      float error_nonweighted_i_left = 0;
      ComputeGradientHessian_ReprojectionError_Planar3Dof(jacobian_transpose_u_left, jacobian_transpose_v_left,
                                                          residual_vector_left, THRES_HUBER, gradient_i_left,
                                                          hessian_i_left, error_i_left, error_nonweighted_i_left);
      ...
      Eigen::Matrix<float, 2, 1> residual_vector_right;
      ComputeJacobianResidual_ReprojectionError_Planar3Dof(
          right_local_position, world_position, matched_right_pixel, fx_right, fy_right, cx_right, cy_right,
          rotation_right_camera_to_base, cos_psi, sin_psi, jacobian_transpose_u_right, jacobian_transpose_v_right,
          residual_vector_right);
      ...
      float error_i_right = 0;
      float error_nonweighted_i_right = 0;
      ComputeGradientHessian_ReprojectionError_Planar3Dof(jacobian_transpose_u_right, jacobian_transpose_v_right,
                                                          residual_vector_right, THRES_HUBER, gradient_i_right,
                                                          hessian_i_right, error_i_right, error_nonweighted_i_right);
...
const std::vector<Eigen::Isometry3f> &PoseOnlyBundleAdjustmentSolver::GetDebugPoses() const { return debug_poses_; }

inline void PoseOnlyBundleAdjustmentSolver::CalculateJtJ_x_6Dof(const Eigen::Matrix<float, 6, 1> &Jt,
...
inline void PoseOnlyBundleAdjustmentSolver::CalculateJtJ_y_6Dof(const Eigen::Matrix<float, 6, 1> &Jt,
                                                                Eigen::Matrix<float, 6, 6> &JtJ_tmp) {
  JtJ_tmp.setZero();
...
inline void PoseOnlyBundleAdjustmentSolver::CalculateJtWJ_x_6Dof(const float weight,
                                                                 const Eigen::Matrix<float, 6, 1> &Jt,
                                                                 Eigen::Matrix<float, 6, 6> &JtJ_tmp) {
  JtJ_tmp.setZero();
...
inline void PoseOnlyBundleAdjustmentSolver::CalculateJtWJ_y_6Dof(const float weight,
                                                                 const Eigen::Matrix<float, 6, 1> &Jt,
                                                                 Eigen::Matrix<float, 6, 6> &JtJ_tmp) {
  JtJ_tmp.setZero();
...
inline void PoseOnlyBundleAdjustmentSolver::AppendToHessian_OnlyUpperTriangle_6Dof(
    const Eigen::Matrix<float, 6, 6> &JtJ_tmp, Eigen::Matrix<float, 6, 6> &JtJ) {
  JtJ(0, 0) += JtJ_tmp(0, 0);
  JtJ(0, 1) += JtJ_tmp(0, 1);
  JtJ(0, 2) += JtJ_tmp(0, 2);
  JtJ(0, 3) += JtJ_tmp(0, 3);
...
inline void PoseOnlyBundleAdjustmentSolver::CalculateJtJ_Planar3Dof(const Eigen::Matrix<float, 3, 1> &Jt,
...
inline void PoseOnlyBundleAdjustmentSolver::CalculateJtWJ_Planar3Dof(const float weight,
                                                                     const Eigen::Matrix<float, 3, 1> &Jt,
                                                                     Eigen::Matrix<float, 3, 3> &JtJ_tmp) {
  JtJ_tmp.setZero();
...
inline void PoseOnlyBundleAdjustmentSolver::AppendToHessian_OnlyUpperTriangle_Planar3Dof(
    const Eigen::Matrix<float, 3, 3> &JtJ_tmp, Eigen::Matrix<float, 3, 3> &JtJ) {
  JtJ(0, 0) += JtJ_tmp(0, 0);
  JtJ(0, 1) += JtJ_tmp(0, 1);
  JtJ(0, 2) += JtJ_tmp(0, 2);
...
inline void PoseOnlyBundleAdjustmentSolver::FillLowerTriangleByUpperTriangle_Planar3Dof(
    Eigen::Matrix<float, 3, 3> &JtJ) {
  JtJ(1, 0) = JtJ(0, 1);
  JtJ(2, 0) = JtJ(0, 2);
...
template <typename T>
void PoseOnlyBundleAdjustmentSolver::CalculateMatrixExpoenetial_se3(const Eigen::Matrix<T, 6, 1> &xi,
...
template <typename T>
void PoseOnlyBundleAdjustmentSolver::CalculateMatrixExpoenetial_so3(const Eigen::Matrix<T, 3, 1> &w,
...
inline void PoseOnlyBundleAdjustmentSolver::WarpPositionList(const Eigen::Isometry3f &pose_target_to_initial,
...
inline void PoseOnlyBundleAdjustmentSolver::ComputeJacobianResidual_ReprojectionError_6Dof(
    const Eigen::Vector3f &local_position, const Eigen::Vector2f &matched_pixel, const float fx, const float fy,
    const float cx, const float cy, Eigen::Matrix<float, 6, 1> &jacobian_matrix_x_transpose,
    Eigen::Matrix<float, 6, 1> &jacobian_matrix_y_transpose, Eigen::Matrix<float, 2, 1> &residual_vector) {
...
inline void PoseOnlyBundleAdjustmentSolver::ComputeGradientHessian_ReprojectionError_6Dof(
    ...
    const Eigen::Matrix<float, 6, 1> &jacobian_matrix_v_transpose, const Eigen::Matrix<float, 2, 1> &residual_vector,
    const float &threshold_huber, Eigen::Matrix<float, 6, 1> &gradient_vector,
    Eigen::Matrix<float, 6, 6> &hessian_matrix, float &error, float &error_nonweighted) {
...
inline void PoseOnlyBundleAdjustmentSolver::ComputeJacobianResidual_ReprojectionError_Planar3Dof(
    const Eigen::Vector3f &target_local_position, const Eigen::Vector3f &world_position,
    const Eigen::Vector2f &matched_pixel, const float fx, const float fy, const float cx, const float cy,
    const Eigen::Matrix<float, 3, 3> &rotation_target_camera_to_base, const float cos_psi, const float sin_psi,
    Eigen::Matrix<float, 3, 1> &jacobian_matrix_u_transpose, Eigen::Matrix<float, 3, 1> &jacobian_matrix_v_transpose,
    Eigen::Matrix<float, 2, 1> &residual_vector) {
...
inline void PoseOnlyBundleAdjustmentSolver::ComputeGradientHessian_ReprojectionError_Planar3Dof(
    ...
    const Eigen::Matrix<float, 3, 1> &jacobian_matrix_v_transpose, const Eigen::Matrix<float, 2, 1> &residual_vector,
    const float &threshold_huber, Eigen::Matrix<float, 3, 1> &gradient_vector,
    Eigen::Matrix<float, 3, 3> &hessian_matrix, float &error, float &error_nonweighted) {
  const auto &Jt_u = jacobian_matrix_u_transpose;
  const auto &Jt_v = jacobian_matrix_v_transpose;
  auto &JtWr = gradient_vector;
  auto &JtWJ = hessian_matrix;
