    ...
    std::vector<Eigen::Vector2f> &pixel_list)
{
  ...
  for (size_t index = 0; index < num_points; ++index)
  {
    ...
    const Eigen::Vector3f local_position = pose_world_to_current.inverse() * world_position;
...
int main()
{
  try
  {
    ...
    MonocularReprojectionErrorCostFunction3DofNumerical::SetPoseBaseToCamera(pose_base_to_camera.cast<double>());
    MonocularReprojectionErrorCostFunction3DofNumerical::SetCameraIntrinsicParameters(fx, fy, cx, cy);
    ...
    for (size_t index = 0; index < num_points; ++index)
    {
      ...
      ceres::CostFunction *cost_function = new ceres::AutoDiffCostFunction<MonocularReprojectionErrorCostFunction3DofNumerical, 2, 3>(
          new MonocularReprojectionErrorCostFunction3DofNumerical(world_position, pixel_matched));
      problem_ceres_numerical.AddResidualBlock(cost_function, nullptr, param_b2b1_ceres_numerical);
    }
    ...
    pose_b2b1_autodiff.linear() << cos(param_b2b1_ceres_numerical[2]), -sin(param_b2b1_ceres_numerical[2]), 0,
        sin(param_b2b1_ceres_numerical[2]), cos(param_b2b1_ceres_numerical[2]), 0, 0, 0, 1;
    pose_b2b1_autodiff.translation() << param_b2b1_ceres_numerical[0], param_b2b1_ceres_numerical[1], 0;
    const Eigen::Isometry3f pose_b1b2_autodiff = pose_b2b1_autodiff.inverse();
    const Eigen::Isometry3f pose_world_to_current_ceres_autodiff = pose_b1b2_autodiff * pose_base_to_camera;
    pose_ceres_autodiff << pose_world_to_current_ceres_autodiff.linear(), pose_world_to_current_ceres_autodiff.translation();
    ...
    std::cout << "Time (Ceres autodiff): " << summary_ceres_numerical.total_time_in_seconds << " [sec]" << std::endl;
