...
int main() {
  try {
    ...
    pose_world_to_current_frame.linear() =
        Eigen::AngleAxisf(-0.12, Eigen::Vector3f::UnitY()).toRotationMatrix();
    ...
    for (size_t index = 0; index < num_points; ++index) {
      ...
      const Eigen::Vector3f left_local_position =
          pose_world_to_current_frame.inverse() * world_position;
      const Eigen::Vector3f right_local_position =
          pose_left_to_right.inverse() * left_local_position;
    ...
    pose_world_to_current_frame_optimized =
        pose_world_to_current_frame_initial_guess;
    ...
    pose_optimizer->SolveStereoPoseOnlyBundleAdjustment6Dof(
        world_position_list, left_pixel_list, right_pixel_list, fx, fy, cx, cy,
        fx, fy, cx, cy, pose_left_to_right,
        pose_world_to_current_frame_optimized, mask_inlier_left,
    ...
    //   debug_pose_list[iter]; std::vector<Eigen::Vector2f>
    //   projected_pixel_list; for (size_t index = 0; index < num_points;
    //   ++index)
    //   {
    ...
    //   cv::Mat image_blank = cv::Mat::zeros(cv::Size(n_cols, n_rows),
    //   CV_8UC3); for (size_t index = 0; index < pixel_list.size(); ++index)
    //   {
    ...
    //     cv::Scalar(255, 0, 0), 1); cv::circle(image_blank,
