...
class PoseOnlyBundleAdjustmentSolver {
 ...
 public:
  bool Solve_Monocular_Planar3Dof(
      ...
      Options options, Summary *summary = nullptr);
 ...
 private:
  inline void CalculateJtJ_x_6Dof(const Eigen::Matrix<float, 6, 1> &Jt,
                                  Eigen::Matrix<float, 6, 6> &JtJ_tmp);
  inline void CalculateJtJ_y_6Dof(const Eigen::Matrix<float, 6, 1> &Jt,
                                  Eigen::Matrix<float, 6, 6> &JtJ_tmp);
  inline void CalculateJtWJ_x_6Dof(const float weight,
                                   const Eigen::Matrix<float, 6, 1> &Jt,
                                   Eigen::Matrix<float, 6, 6> &JtJ_tmp);
  inline void CalculateJtWJ_y_6Dof(const float weight,
                                   const Eigen::Matrix<float, 6, 1> &Jt,
                                   Eigen::Matrix<float, 6, 6> &JtJ_tmp);
  inline void AppendToHessian_OnlyUpperTriangle_6Dof(
  ...
  inline void CalculateJtJ_Planar3Dof(const Eigen::Matrix<float, 3, 1> &Jt,
                                      Eigen::Matrix<float, 3, 3> &JtJ_tmp);
  inline void CalculateJtWJ_Planar3Dof(const float weight,
                                       const Eigen::Matrix<float, 3, 1> &Jt,
                                       Eigen::Matrix<float, 3, 3> &JtJ_tmp);
  inline void AppendToHessian_OnlyUpperTriangle_Planar3Dof(
      const Eigen::Matrix<float, 3, 3> &JtJ_tmp,
      Eigen::Matrix<float, 3, 3> &JtJ);
  inline void FillLowerTriangleByUpperTriangle_Planar3Dof(
      Eigen::Matrix<float, 3, 3> &JtJ);

 private:
  template <typename T>
  void CalculateMatrixExpoenetial_se3(const Eigen::Matrix<T, 6, 1> &xi,
                                      Eigen::Transform<T, 3, 1> &pose);
  template <typename T>
  void CalculateMatrixExpoenetial_so3(const Eigen::Matrix<T, 3, 1> &w,
