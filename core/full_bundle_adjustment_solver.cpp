...
namespace ba_solver
{
  ...
  void FullBundleAdjustmentSolver::AddCamera(const _BA_Camera &camera)
  {
    ...
    std::cout << "  fx: " << camera_scaled.fx << ", fy: " << camera_scaled.fy << ", cx: " << camera_scaled.cx << ", cy: " << camera_scaled.cy << "\n";
  ...
  void FullBundleAdjustmentSolver::FinalizeParameters()
  {
    ...
    {
      if (fixed_original_pose_set_.count(original_pose) > 0)
        continue;
    ...
    {
      if (fixed_original_point_set_.count(original_point) > 0)
        continue;
  ...
  std::string FullBundleAdjustmentSolver::GetSolverStatistics() const
  {
    ...
    std::cout << "|                Jacobian size: " << 6 * num_observation_ << " rows x " << 3 * M_optimize_ + 6 * N_optimize_ << " cols" << std::endl;
  ...
  void FullBundleAdjustmentSolver::SetProblemSize()
  {
    ...
    for (_BA_Index j = 0; j < N_optimize_; ++j)
      B_[j].resize(M_optimize_, _BA_Mat63::Zero()); // 6x3, N_opt X M blocks

    Bt_.resize(M_optimize_);
    for (_BA_Index i = 0; i < M_optimize_; ++i)
      Bt_[i].resize(N_optimize_, _BA_Mat36::Zero()); // 3x6, N_opt X M blocks
    ...
    for (_BA_Index i = 0; i < M_optimize_; ++i)
      CinvBt_[i].resize(N_optimize_, _BA_Mat36::Zero());
  ...
  void FullBundleAdjustmentSolver::CheckPoseAndPointConnectivity()
  {
    for (size_t j_opt = 0; j_opt < j_opt_to_all_point_.size(); ++j_opt)
    {
      ...
      if (num_observed_point < 4)
        std::cerr << TEXT_YELLOW(std::to_string(j_opt) + "-th pose: It might diverge because some frames have insufficient related points.") << std::endl;
    }
    for (size_t i_opt = 0; i_opt < i_opt_to_all_pose_.size(); ++i_opt)
    {
      ...
      if (num_related_pose < 4)
        std::cerr << TEXT_YELLOW(std::to_string(i_opt) + "-th point: It might diverge because some points have insufficient related poses.") << std::endl;
    }
  }
  ...
  bool FullBundleAdjustmentSolver::Solve(Options options, Summary *summary)
  {
    ...
    for (int iteration = 0; iteration < MAX_ITERATION; ++iteration)
    {
      ...
      for (_BA_Index j = 0; j < N_optimize_; ++j)
        for (_BA_Index k = j; k < N_optimize_; ++k)
      ...
      {
        _BA_Index idx1 = 0;
        for (_BA_Index u = 0; u < N_optimize_; ++u, idx1 += 6)
      ...
      */
      ...
      for (const auto &xj : x_)
        step_size_pose_norm += xj.norm();
      for (const auto &yi : y_)
        step_size_point_norm += yi.norm();

      const auto average_delta_error = delta_error / static_cast<double>(num_observation_);
      const auto average_total_step_size = (step_size_pose_norm + step_size_point_norm) / static_cast<double>(N_optimize_ + M_optimize_);
