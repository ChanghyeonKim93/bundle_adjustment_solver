...
bool PoseOnlyBundleAdjustmentSolver::Solve_Monocular_6Dof(
    ...
    const std::vector<Eigen::Vector2f> &matched_pixel_list, const float fx,
  ...
  if (reference_position_list.size() != matched_pixel_list.size())
    throw std::runtime_error(
        ...
        "SolveMonocularPoseOnlyBundleAdjustment6Dof(), "
        "world_position_list.size() "
  ...
  Eigen::Isometry3f pose_camera_to_world_optimized =
      reference_to_current_pose.inverse();
  ...
  for (int iteration = 0; iteration < MAX_ITERATION; ++iteration) {
    ...
    for (size_t index = 0; index < n_pts; ++index) {
      ...
      ComputeJacobianResidual_ReprojectionError_6Dof(
          local_position, matched_pixel, fx, fy, cx, cy, jacobian_transpose_u,
          jacobian_transpose_v, residual_vector);
    ...
    pose_camera_to_world_optimized =
        delta_pose * pose_camera_to_world_optimized;
    ...
    if (delta_xi.norm() < THRES_DELTA_XI ||
        delta_error < THRES_DELTA_ERROR)  // Early convergence
    {
      is_converged = true;
      break;
    }
  ...
  if (!std::isnan(pose_camera_to_world_optimized.linear().norm())) {
    ...
    std::cout << "!! WARNING !! poseonly BA yields NAN value!!"
              << ", Tcw_optimized: \n"
              << pose_camera_to_world_optimized.linear() << " "
              << pose_camera_to_world_optimized.translation() << "\n";
...
bool PoseOnlyBundleAdjustmentSolver::Solve_Stereo_6Dof(
    ...
    const float fx_left, const float fy_left, const float cx_left,
    const float cy_left, const float fx_right, const float fy_right,
    const float cx_right, const float cy_right,
    const Eigen::Isometry3f &left_to_right_pose,
    Eigen::Isometry3f &reference_to_current_left_pose,
    std::vector<bool> &mask_inlier_left, std::vector<bool> &mask_inlier_right,
  ...
  if (reference_position_list.size() != matched_left_pixel_list.size())
    throw std::runtime_error(
        "In "
        "PoseOnlyBundleAdjustmentSolver::"
        "SolveStereoPoseOnlyBundleAdjustment6Dof(), world_position_list.size() "
        "!= "
  ...
  Eigen::Isometry3f pose_camera_to_world_optimized =
      reference_to_current_left_pose.inverse();
  ...
  for (int iter = 0; iter < MAX_ITERATION; ++iter) {
    ...
    const Eigen::Isometry3f pose_right_camera_to_world_optimized =
        pose_right_to_left * pose_camera_to_world_optimized;
    ...
    for (size_t index = 0; index < n_pts; ++index) {
      const Eigen::Vector2f &matched_left_pixel =
          matched_left_pixel_list[index];
      const Eigen::Vector2f &matched_right_pixel =
          matched_right_pixel_list[index];
      const Eigen::Vector3f &left_local_position =
          warped_left_position_list[index];
      const Eigen::Vector3f &right_local_position =
          warped_right_position_list[index];
      ...
      ComputeJacobianResidual_ReprojectionError_6Dof(
          left_local_position, matched_left_pixel, fx_left, fy_left, cx_left,
      ...
      ComputeGradientHessian_ReprojectionError_6Dof(
          jacobian_transpose_u_left, jacobian_transpose_v_left,
      ...
      ComputeJacobianResidual_ReprojectionError_6Dof(
          right_local_position, matched_right_pixel, fx_right, fy_right,
      ...
      ComputeGradientHessian_ReprojectionError_6Dof(
          jacobian_transpose_u_right, jacobian_transpose_v_right,
          residual_vector_right, THRES_HUBER, gradient_i_right, hessian_i_right,
          error_i_right, error_nonweighted_i_right);
...
bool PoseOnlyBundleAdjustmentSolver::Solve_Monocular_Planar3Dof(
    ...
    Eigen::Isometry3f &pose_world_to_current, std::vector<bool> &mask_inlier,
    Options options, Summary *summary) {
  ...
  if (world_position_list.size() != matched_pixel_list.size())
    throw std::runtime_error(
        "In "
        "PoseOnlyBundleAdjustmentSolver::"
        "SolveMonocularPoseOnlyBundleAdjustment3Dof(), "
        "world_position_list.size() "
        "!= current_pixel_list.size()");
  ...
  const Eigen::Isometry3f pose_b2b1 =
      pose_base_to_camera * pose_c2c1_prior * pose_camera_to_base;
  ...
  for (int iteration = 0; iteration < MAX_ITERATION; ++iteration) {
    ...
    pose_b2b1_optimized.linear() << cos_psi, -sin_psi, 0, sin_psi, cos_psi, 0,
        0, 0, 1;
    ...
    for (size_t index = 0; index < n_pts; ++index) {
      ...
      ComputeJacobianResidual_ReprojectionError_Planar3Dof(
          local_position, world_position, matched_pixel, fx, fy, cx, cy, R_cb,
          cos_psi, sin_psi, jacobian_transpose_u, jacobian_transpose_v,
          residual_i);
      ...
      ComputeGradientHessian_ReprojectionError_Planar3Dof(
          jacobian_transpose_u, jacobian_transpose_v, residual_i, THRES_HUBER,
    ...
    delta_pose.linear() << cos(delta_psi), -sin(delta_psi), 0, sin(delta_psi),
        cos(delta_psi), 0, 0, 0, 1;
    ...
    pose_world_to_current_optimized =
        pose_b2b1_optimized.inverse() * pose_base_to_camera;
    debug_poses_.push_back(pose_world_to_current_optimized);

    err_curr *= (inverse_n_pts * 0.5f);
    const float delta_error = abs(err_curr - err_prev);

    if (delta_param.norm() < THRES_DELTA_XI ||
        delta_error < THRES_DELTA_ERROR) {
      is_converged = true;
      // std::cout << "    Early convergence. stops at iteration " << iter << ",
      // error: " << err_curr << ", delta_error: " << delta_error << ",
      // step_size: " << delta_xi.norm() << ", # invalid: " << count_invalid <<
      // "\n";
      break;
    }
...
bool PoseOnlyBundleAdjustmentSolver::Solve_Stereo_Planar3Dof(
  ...
  const Eigen::Matrix3f rotation_left_camera_to_base =
      pose_camera_to_base.linear();
  const Eigen::Matrix3f rotation_right_camera_to_base =
      pose_right_to_left.linear() * pose_camera_to_base.linear();

  // Warp the world position to the last frame
  const Eigen::Isometry3f pose_c2c1_prior =
      world_to_current_pose.inverse() * world_to_last_pose;
  const Eigen::Isometry3f pose_b2b1 =
      base_to_camera_pose * pose_c2c1_prior * pose_camera_to_base;
  ...
  for (int iteration = 0; iteration < MAX_ITERATION; ++iteration) {
    ...
    pose_b2b1_optimized.linear() << cos_psi, -sin_psi, 0, sin_psi, cos_psi, 0,
        0, 0, 1;
    pose_b2b1_optimized.translation() << x_b2b1_optimized, y_b2b1_optimized, 0;
    const auto pose_left_c2b1_optimized =
        pose_camera_to_base * pose_b2b1_optimized;
    const auto pose_right_c2b1_optimized =
        pose_right_to_left * pose_left_c2b1_optimized;
    ...
    for (size_t index = 0; index < n_pts; ++index) {
      ...
      Eigen::Matrix<float, 2, 1> residual_vector_left;
      ComputeJacobianResidual_ReprojectionError_Planar3Dof(
          left_local_position, world_position, matched_left_pixel, fx_left,
          fy_left, cx_left, cy_left, rotation_left_camera_to_base, cos_psi,
          sin_psi, jacobian_transpose_u_left, jacobian_transpose_v_left,
          residual_vector_left);
      ...
      Eigen::Matrix<float, 2, 1> residual_vector_right;
      ComputeJacobianResidual_ReprojectionError_Planar3Dof(
          right_local_position, world_position, matched_right_pixel, fx_right,
          fy_right, cx_right, cy_right, rotation_right_camera_to_base, cos_psi,
          sin_psi, jacobian_transpose_u_right, jacobian_transpose_v_right,
    ...
    pose_world_to_current_optimized =
        pose_b2b1_optimized.inverse() * base_to_camera_pose;
    debug_poses_.push_back(pose_world_to_current_optimized);
...
const std::vector<Eigen::Isometry3f> &
PoseOnlyBundleAdjustmentSolver::GetDebugPoses() const {
  return debug_poses_;
}

inline void PoseOnlyBundleAdjustmentSolver::CalculateJtJ_x_6Dof(
    const Eigen::Matrix<float, 6, 1> &Jt, Eigen::Matrix<float, 6, 6> &JtJ_tmp) {
  JtJ_tmp.setZero();
...
inline void PoseOnlyBundleAdjustmentSolver::CalculateJtJ_y_6Dof(
    const Eigen::Matrix<float, 6, 1> &Jt, Eigen::Matrix<float, 6, 6> &JtJ_tmp) {
  JtJ_tmp.setZero();
...
inline void PoseOnlyBundleAdjustmentSolver::CalculateJtWJ_x_6Dof(
    const float weight, const Eigen::Matrix<float, 6, 1> &Jt,
  ...
  // Product the weight
  wJt << weight * Jt(0), weight * Jt(1), weight * Jt(2), weight * Jt(3),
      weight * Jt(4), weight * Jt(5);
...
inline void PoseOnlyBundleAdjustmentSolver::CalculateJtWJ_y_6Dof(
    const float weight, const Eigen::Matrix<float, 6, 1> &Jt,
    Eigen::Matrix<float, 6, 6> &JtJ_tmp) {
  JtJ_tmp.setZero();
...
inline void
PoseOnlyBundleAdjustmentSolver::AppendToHessian_OnlyUpperTriangle_6Dof(
    const Eigen::Matrix<float, 6, 6> &JtJ_tmp,
...
inline void PoseOnlyBundleAdjustmentSolver::FillLowerTriangle_6Dof(
    Eigen::Matrix<float, 6, 6> &JtJ) {
  JtJ(1, 0) = JtJ(0, 1);
  JtJ(2, 0) = JtJ(0, 2);
  JtJ(3, 0) = JtJ(0, 3);
...
inline void PoseOnlyBundleAdjustmentSolver::CalculateJtJ_Planar3Dof(
    const Eigen::Matrix<float, 3, 1> &Jt, Eigen::Matrix<float, 3, 3> &JtJ_tmp) {
  JtJ_tmp.setZero();
...
inline void PoseOnlyBundleAdjustmentSolver::CalculateJtWJ_Planar3Dof(
    const float weight, const Eigen::Matrix<float, 3, 1> &Jt,
...
inline void
PoseOnlyBundleAdjustmentSolver::AppendToHessian_OnlyUpperTriangle_Planar3Dof(
    const Eigen::Matrix<float, 3, 3> &JtJ_tmp,
...
inline void
PoseOnlyBundleAdjustmentSolver::FillLowerTriangleByUpperTriangle_Planar3Dof(
    Eigen::Matrix<float, 3, 3> &JtJ) {
  JtJ(1, 0) = JtJ(0, 1);
  JtJ(2, 0) = JtJ(0, 2);
...
template <typename T>
void PoseOnlyBundleAdjustmentSolver::CalculateMatrixExpoenetial_se3(
...
template <typename T>
void PoseOnlyBundleAdjustmentSolver::CalculateMatrixExpoenetial_so3(
    const Eigen::Matrix<T, 3, 1> &w, Eigen::Matrix<T, 3, 3> &R) {
  // initialize variables
  T theta = 0.0;
...
inline void
PoseOnlyBundleAdjustmentSolver::ComputeJacobianResidual_ReprojectionError_6Dof(
    const Eigen::Vector3f &local_position, const Eigen::Vector2f &matched_pixel,
...
inline void
PoseOnlyBundleAdjustmentSolver::ComputeGradientHessian_ReprojectionError_6Dof(
    ...
    Eigen::Matrix<float, 6, 6> &hessian_matrix, float &error,
  ...
  // Huber weight calculation by the Manhattan distance
  float weight = 1.0f;
  const auto abs_residual_sum =
      abs(residual_vector(0)) + abs(residual_vector(1));
  ...
  if (abs_residual_sum >= threshold_huber) {
    ...
    this->CalculateJtWJ_x_6Dof(
        weight, Jt_u,
        JtJ_tmp);  // JtWJ.noalias()  += weight*(Jt*Jt.transpose());
    ...
    this->CalculateJtJ_x_6Dof(
        Jt_u, JtJ_tmp);  // JtWJ.noalias()  += weight*(Jt*Jt.transpose());
    // JtWJ.noalias() += JtJ_tmp;
    AppendToHessian_OnlyUpperTriangle_6Dof(JtJ_tmp, JtWJ);
    ...
    this->CalculateJtJ_y_6Dof(
        Jt_v, JtJ_tmp);  // JtWJ.noalias()  += weight*(Jt*Jt.transpose());
    // JtWJ.noalias() += JtJ_tmp;
    AppendToHessian_OnlyUpperTriangle_6Dof(JtJ_tmp, JtWJ);
...
inline void PoseOnlyBundleAdjustmentSolver::
    ComputeJacobianResidual_ReprojectionError_Planar3Dof(
        const Eigen::Vector3f &target_local_position,
        const Eigen::Vector3f &world_position,
        const Eigen::Vector2f &matched_pixel, const float fx, const float fy,
  ...
  jacobian_matrix_u_transpose(2) =
      jacobian_matrix_u_transpose(0) * A + jacobian_matrix_u_transpose(1) * B;

  jacobian_matrix_v_transpose(0) = beta_1_r21 + beta_2_r31;
  jacobian_matrix_v_transpose(1) = beta_1_r22 + beta_2_r32;
  jacobian_matrix_v_transpose(2) =
      jacobian_matrix_v_transpose(0) * A + jacobian_matrix_v_transpose(1) * B;
}
inline void PoseOnlyBundleAdjustmentSolver::
    ComputeGradientHessian_ReprojectionError_Planar3Dof(
        ...
        Eigen::Matrix<float, 3, 3> &hessian_matrix, float &error,
        float &error_nonweighted) {
  const auto &Jt_u = jacobian_matrix_u_transpose;
  const auto &Jt_v = jacobian_matrix_v_transpose;
  auto &JtWr = gradient_vector;
